...
class AnyResult<R> internal constructor(
  ...
  /**
   * Returns the result cast to a good result while throwing
   * an exception if it is a bad result
   */
  fun toGood(): GoodResult<R>
    = good ?: throw Exception("Can't return a bad result as if it were a good one")

  /**
   * Returns the result cast to a bad result while throwing
   * an exception if it is a good result
   */
  ...
  /**
   * Returns the input result if its a good result else
  ...
  /**
   * Returns the input result if its a bad result else
   * returns the result of the function
   *
  ...
  /**
   * Invokes the function and returns the result if its a good result
   * else returns a new bad result
   *
   * @param[f] Function to invoke
   */
  ...
  /**
   * Invokes the function and returns the result if its a bad result
  ...
  /**
   * Throws the exception if the result is bad creating
   * an exception if one is not present
   */
  ...
  /**
   * Returns the result if its a good result else invokes
   * the function
   *
   * @param[f] Function to invoke
   */
  ...
  /**
   * Unwraps the value throwing an exception if the
   * result is bad
   */
  ...
  companion object {
     ...
     *
     * If the function throws an exception it is caught and wrapped
     * in a [BadResult]
     *
...
class GoodResult<R> (
  ...
  /**
   * Invokes the function and returns the result if its a good result
   * but empty else returns this result
   *
   * @param[f] Function to invoke
   */
...
class BadResult<R>(
  ...
  /**
   * Throws the bad result as an exception creating one
   * if it's null
   */
  fun throwEx() {
    exception ?: throw Exception(message)
  ...
  /**
   * Throws the bad result as an exception but always
   * wraps the exception first so the message is always
   * included
   */
...
/**
 * Wraps the value in an [GoodResult] if the result
 * is not null else a [BadResult]
 *
 ...
 * @param[R] Type of the result
 */
inline fun <R> AnyResult<R?>.mapIfNull(f: () -> AnyResult<R?>): AnyResult<R?> = when {
  good == null -> this
  good.result != null -> this
  else -> f()
}
...
/**
 * Invokes the function and returns the result if its bad
 * or null result else returns this result
 *
 * @param[f] Function to invoke
 * @param[R] Type of the result
 */
inline fun <R> AnyResult<R?>.mapIfBadOrNull(f: () -> AnyResult<R?>): AnyResult<R?> = when {
  good?.result != null -> this
  else -> f()
}

/**
 * Returns the result if its good and value present else
...
/**
 * Converts the result to a good [AnyResult] with a [BadResult]
 * being produced if the result value is null
 *
